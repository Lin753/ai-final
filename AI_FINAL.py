...
def get_songs(path):
    ...
    for f in tqdm(files):
        try:
            song = np.array(manipulate.midiToNoteStateMatrix(f))
...
lowest_note = manipulate.lowerBound  # the index of the lowest note on the piano roll
highest_note = manipulate.upperBound  # the index of the highest note on the piano roll
...
with tf.Session() as sess:
    ...
    for i in range(sample.shape[0]):
        ...
        manipulate.noteStateMatrixToMidi(S, "out/generated_chord_{}".format(i))
